...
class UjiKuatService {
  ...
  double hitungLuasPermukaanKubus(double sisiKubus) {
    double sisiCm = sisiKubus / 10.0;
    double luasCm2 = sisiCm * sisiCm;
    return luasCm2;
  }
  ...
  String generateKeterangan(
    ...
    {double? sisiKubus, double? diameter, double? beratBendaUji,
    ...
    if (standar == 'SNI' && sisiKubus != null) {
      hasil.write('Pengujian menggunakan benda uji KUBUS ${sisiKubus.toStringAsFixed(0)}×${sisiKubus.toStringAsFixed(0)}×${sisiKubus.toStringAsFixed(0)} mm. ');
  ...
  UjiKuatResult prosesUjiKuat(UjiKuatData data, String standar) {
    ...
    if (standar == 'SNI' && data.sisiKubus != null) {
     
      
      luasPermukaan = hitungLuasPermukaanKubus(data.sisiKubus!);
      
     
      double kuatTekanUmurUjiKgCm2 = hitungKuatTekanKubus(data.beban, luasPermukaan);
